...
@triton.jit
def _rmsnorm_bwd_dg_reduce_triton(dg_in_ptr, dg_out_ptr, dg_in_stride, n_rows, n_cols, BLOCK_SIZE: tl.constexpr):
...
def te_rmsnorm_bwd_triton(dz, x, rsigma, gamma, zero_centered_gamma):
    ...
    _rmsnorm_bwd_triton[grid_bwd](dz_, x_, gamma_, rsigma_, dx, dg_tmp, x_.stride(0), dz_.stride(0), M, N, zero_centered_gamma, blk_size, USE_BLOCKED, NUM_PRGMS)

    grid_reduce = lambda meta: (triton.cdiv(N, blk_size), )
    _rmsnorm_bwd_dg_reduce_triton[grid_reduce](dg_tmp, dgamma, dg_tmp.stride(0), M, N, blk_size)
